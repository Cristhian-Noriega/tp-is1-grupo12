...
export const productsUtils = () => {
  ...
  const deleteProduct = async (productId) => {
    try {
      console.log(`esto eliminando el product con id: ${productId}`);
  ...
  const createProduct = async (product) => {
    try {
      ...
      console.log(product);
      const productResponse = await productsAdminService.create(product);
  ...
  const editProductStock = async (productId, stock) => {
    try {
      ...
      console.log(productId);
      const productStock = await productsAdminService.updateStock(productId, {
        stock,
      });
      console.log("se hizo la request");
      console.log(productStock);
  ...
  return {
    ...
    showMessage,
    setShowMessage,
    handleGetProductByAttributes,
    handleGetStockByProductId,
    editProductStock,
  };
};
